...
public class CartController {
    ...
    @ApiResponses({
        @ApiResponse(code = 404, message = "Cart not found")
        ,@ApiResponse(code = 409, message = "No product registred")
        ,@ApiResponse(code = 200, message = "Exits one product at least")
    ...
    public ResponseEntity<List<Product>> getRecomendedProductsByUser(@PathVariable("idUser") final Long idUser) throws SQLException {
        ...
        if(listCart.isEmpty()){
            throw new CartController.FoundException("USER DONT HAVE ANY PREVIUS ORDER");
        }
        if (prodRec.size()==0) {
           throw new CartController.FoundException("DONT HAVE PRODUCT RELATIONATED TO YOUR PREVIUS ORDERS");
        }
